  ...
  setup(props) {
    ...
    const shareItem = (item: InterfaceItem) => {
      void share(
        ...
        `${config.app.host || 'portal.msmusic.edu.sg'}/items/${item.documentId}`
      );
    };
    ...
    const getItemCollections = () => {
      const collections: InterfaceCollection[] = collectionsState.collections.filter(
        collection => {
          return (
            collection.items.find(item => item.id === props.item.id) !==
            undefined
          );
        }
      );
    ...
    const isInACollection = () => {
      const collectionIds = collectionsState.collections.map(
        collection => collection.id
      );
      const intersect = itemCollections.value.filter(value =>
        collectionIds.includes(value)
      );
      return intersect.length > 0;
    };

    watch(() => collectionsState.collections, getItemCollections);
