<template>
  <q-page id="page">
    ...
    <Error404 v-else-if="is404" />
    <section v-else-if="title !== undefined && content !== undefined">
      <header v-if="banner">
        <q-img :src="banner.url" :ratio="4 / 1">
          <div v-if="bannerHeader" class="absolute-full flex">
            <div class="container flex" style="width: 100%">
              <div class="row flex items-center">
                <div class="col-12 col-md-10 col-lg-8">
                  <div v-if="bannerCaption" class="text-h4">
                    {{ bannerCaption }}<br />
                  </div>
      ...
      <div class="container">
        <div class="row">
          <div class="col-12 col-sm-10 col-md-8 q-py-xl">
            <h1 class="text-h3 text-bold text-accent q-mt-md q-mb-xl">
              {{ title }}
            </h1>
...
export default defineComponent({
  ...
  setup(_props, ctx) {
    ...
    const render = async () => {
      const page: InterfacePage | undefined = await fetchPage(
        ctx.root.$route.path.replace(/\//g, '')
      );
      if (page !== undefined) {
        is404.value = false;
    ...
    watch(() => ctx.root.$route.path, render);
