<template>
  <q-page class="container" v-if="item !== undefined">
    <q-banner v-if="preview" rounded class="bg-orange text-white text-center">
      <b>Note: This Is A Preview</b>
    </q-banner>
    <div :class="'row ' + (isMobile ? 'q-pt-md' : 'q-pt-xl')">
      <div
        :class="
          'column col-xs-12 col-sm-12 col-md-4 col-lg-3 ' +
            (isMobile ? 'items-center' : '')
        "
      >
        <ItemCategories v-if="isMobile" :item="item" />
        <q-img
          :src="
            item &&
            item.featured_image &&
            typeof item.featured_image.url === 'string'
              ...
              : item.categories[0].featured_image.url || '/assets/musicfile.jpg'
          "
      ...
      <div
        :class="
          'column col-xs-12 col-sm-12 col-md-8 col-lg-9 ' +
            (isMobile ? 'q-mt-md' : 'q-pl-lg')
        "
      >
  ...
  <q-page v-else class="flex items-center">
...
export default defineComponent({
  ...
  setup(_props, ctx) {
    ...
    const updateItem = () => {
      item.value = state.items.find(item => String(item.documentId) === id);
    };
