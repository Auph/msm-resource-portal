...
import {
  ...
  InterfaceStateAuthenticationLogin
} from 'src/interfaces';
...
const useAuthentication = () => {
   ...
   */
  const login = (encodedRedirectUriComponent: string | undefined) => {
    resetErrors();
    loading.value = true;
    ...
    axios
      ...
      .then((response: AxiosResponse) => {
        ...
        if (encodedRedirectUriComponent) {
          const decodedRedirectUriComponent = decodeURIComponent(
            encodedRedirectUriComponent
          );
            ...
            for (const message of single.messages) {
              switch (message.id) {
                ...
                case 'Auth.form.error.confirmed':
                  errors.others = `${
                    message.message
                  }<br/><small><a href="/verify-email?email=${String(
                    state.email
                  )}">Resend Email?</a></small>`;
                  break;
                default:
