...
const fetchCollectionCategories = async (
  ...
  try {
    const response: AxiosResponse = await axios.get(
      `${config.apiUrl}/collection-categories?populate=*`,
      options
    );
...
const useCollectionCategories = () => {
  ...
  const getCollectionCategories = async () => {
    const collectionCategories:
      | InterfaceCollectionCategory[]
      | undefined = await fetchCollectionCategories();
...
export { fetchCollectionCategories, useCollectionCategories };
