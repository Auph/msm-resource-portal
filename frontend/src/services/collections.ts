...
const useCollections = () => {
  ...
  const createCollection = async (
    ...
    const newCollection:
      | InterfaceCollection
      | undefined = await createNewCollection(title);
  ...
  const getCollections = async () => {
    const collections:
      | InterfaceCollection[]
      | undefined = await fetchCollections();
  ...
  const getCollection = async (id: number | string) => {
    const collection: InterfaceCollection | undefined = await fetchCollection(
      id
    );
  ...
  const updateCollection = async (
    ...
    const collection: InterfaceCollection | undefined = await editCollection(
      id,
      attributes
    );
  ...
  const deleteCollection = async (
    ...
    const deletedCollection:
      | InterfaceCollection
      | undefined = await destroyCollection(collectionId);
  ...
  const addItem = async (
    ...
    if (collectionIndex === -1) {
      ...
      const items: number[] = state.collections[collectionIndex].items.map(
        item => item.id
      );
      items.push(itemId);
  ...
  const removeItem = async (
    ...
    if (collectionIndex === -1) {
      ...
      const items: number[] = state.collections[collectionIndex].items
        .map(item => item.id)
